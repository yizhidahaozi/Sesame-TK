 ...
 */
class Captcha2Handler : PageManager.ActivityFocusHandler {
    ...
    private fun handleSlideCaptcha(activity: Activity, root: ViewImage): Boolean {
        return try {
           Log.debug("CaptchaHandler", "========== 开始处理API滑动验证码 ==========")
            // 先检测是否有"向右滑动验证"文字
            ...
            val slideRect = slideTextParent.locationOnScreen()
             Log.debug("CaptchaHandler", "滑动区域位置: ${slideRect.contentToString()}")
                ...
                try {
                    ...
                    val existingPath = DataStore.get("slide_path_API", IntArray::class.java)
                    
                    // 只有路径发生变化时才保存
                    if (existingPath == null || !existingPath.contentEquals(slidePath)) {
                        DataStore.put("slide_path_API", slidePath)
