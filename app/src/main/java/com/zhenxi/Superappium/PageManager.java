...
public class PageManager {
    ...
    static {
        ...
        // 不能直接废掉，部分app存在对这个flag的检测，如果你的反射被阻断，请使用 { @link com.zhenxi.com.zhenxi.Superappium.xpcompat.FreeReflection}
        // com.zhenxi.com.zhenxi.Superappium.xpcompat.CompatHelpers 中默认使用 FreeReflection进行反射操作
     ...
     * 对话框弹窗的回调
     */
    private static final AlertDialogShowListener mAfterAlertDialogShowListener = null;
    ...
    public static ViewImage tryGetTopView(String xpath) {
        ...
        for (WeakReference<PopupWindow> popupWindowWeakReference : PageManager.getPopWindos()) {
            ...
            View mDecorView = CompatHelpers.getObjectField(popupWindow, "mDecorView");
            if (mDecorView == null) {
                continue;
            }
            if (mDecorView.getVisibility() != View.VISIBLE) {
